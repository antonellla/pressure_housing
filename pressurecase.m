function [ ]= pressurecase(mat_index,len,wall,rad,water)
%
...
% Set density (freshwater vs seawater)
if(water == 1)
...
% Compute depths
depth_inf = P_cr_inf / (9.81 * density);
depth_vm = P_cr_vm / (9.81 * density);
depth_dtmb = P_cr_dtmb / (9.81 * density);

% Display results
fprintf('\nCritical buckling pressures for thin-walled cylinder:\n\n');
fprintf('Critical Pressure (infinite cylinder): %f (psi)\n', P_cr_inf);
fprintf('Corresponding Depth: %i ft (%i m)\n', depth_inf, depth_inf*3.28);
fprintf('Critical Pressure (Von Mises): %f (psi)\n', P_cr_vm);
fprintf('Corresponding Depth: %i ft (%i m)\n', depth_vm, depth_vm*3.28);
fprintf('Critical Pressure (DTMB): %f (psi)\n', P_cr_dtmb);
fprintf('Corresponding Depth: %i ft (%i m)\n', depth_dtmb, depth_dtmb*3.28);
